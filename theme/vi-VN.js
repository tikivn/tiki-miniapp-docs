module.exports = {
  locale: 'vi-VN',
  messages: {
    'app.header.menu.home': 'Trang chủ',
    'app.header.menu.getting-started': 'Bắt đầu',
    'app.header.menu.framework': 'Framework',
    'app.header.menu.component': 'Components',
    'app.header.menu.api': 'API',
    'app.header.menu.ide': 'Studio',
    'app.header.menu.design': 'Design',
    'app.home.introduce': 'Xây dựng mọi ứng dụng cùng Tiki.',
    'app.home.join-program': 'Tải về MiniApp Studio',
    'app.home.getting-started': 'Hướng dẫn bắt đầu',
    'app.footer.lang': 'English',
    'app.footer.join-program': 'Phát triển ứng dụng của bạn',
  },
};
